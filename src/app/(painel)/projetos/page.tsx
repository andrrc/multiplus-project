...
import { IndicadorDiasRestantes } from "@/ui/indicador-dias-restantes";
  ...
  return (
    <div className="w-full max-w-[1120px]">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="font-[family-name:var(--font-interface)] text-[11px] font-semibold uppercase tracking-[0.12em] text-azul-esc">Controle de projetos</p>
          <h1 className="mt-1 text-[28px]">Projetos</h1>
          <p className="mt-1.5 text-[15px] text-cinza">Acompanhe clientes, prazos e entregas em um só lugar.</p>
        </div>
      ...
      {projetos.length === 0 ? (
        <div className="mt-8 rounded-[3px] border border-dashed border-linha bg-branco px-8 py-14 text-center text-[16px] text-cinza">Nenhum projeto encontrado. Cadastre o primeiro para começar a acompanhar prazos ambientais.</div>
      ) : <>
        <ul className="mt-7 flex flex-col gap-3 md:hidden">
          {projetos.map((p) => (
            <li key={p.id}>
              <article className="relative block rounded-[3px] border border-linha bg-branco px-4 py-4 hover:border-azul">
                <Link href={`/projetos/${p.id}`} aria-label={`Abrir projeto ${p.nome}`} className="absolute inset-0 z-0 rounded-[3px]">
                  <span className="sr-only">Abrir projeto {p.nome}</span>
                </Link>
                <div className="pointer-events-none relative z-10">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-[family-name:var(--font-interface)] font-semibold text-tinta">{p.nome}</p>
                      <Link href={`/clientes/${p.cliente.id}`} className="pointer-events-auto relative z-20 mt-1 inline-block text-[14px] text-azul-esc hover:underline">{p.cliente.razaoSocial}</Link>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-2 text-[13px] text-cinza"><span>Dias restantes:</span><IndicadorDiasRestantes prazo={p.dataPrevistaConclusao} /></div>
                </div>
              </article>
            </li>
          ))}
          ...
          <tbody>
            {projetos.map((p) => (
              <tr key={p.id} className={`border-t border-linha hover:bg-verde-cl ${p.ativo ? "bg-branco" : "bg-papel"}`}>
                ...
                <td className="p-0"><Link href={`/projetos/${p.id}`} className="block px-5 py-4"><IndicadorDiasRestantes prazo={p.dataPrevistaConclusao} /></Link></td>
