...
import { alterarStatusProjetoAction, alterarStatusTarefaAction, definirAtivoProjetoAction, criarDocumentoProjetoAction } from "../actions";
import { Comentarios } from "../comentarios";
import { StatusCardInline, StatusInline } from "@/ui/status-inline";
...
<div className="mt-4 grid gap-x-8 gap-y-4 border-b border-linha pb-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
  ...
  <div className="flex min-w-0 flex-col items-start gap-3 sm:items-end">
    {projeto.ativo && <StatusCardInline status={projeto.status} label={rotuloStatus} opcoes={Object.entries(projetoStatus).map(([valor, label]) => ({ valor, label }))} action={alterarStatusProjetoAction.bind(null, id)} />}
    {!projeto.ativo && <Etiqueta tom="apagado">Desativado</Etiqueta>}
